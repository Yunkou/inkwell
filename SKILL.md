...
- ` ``` ` → 围栏代码块（带可选语言标签，渲染为 `.code-block` 卡片）

**Fenced code blocks** (`chapter.content` only):

​```ts
interface Report { title: string; chapters: Chapter[] }
​```

- Triple backticks open/close the block; everything in between is **escaped** (no HTML injection).
- Optional language tag after the opening fence (`ts`, `bash`, `json`, …) is shown as a small stone-colored label in the top-right corner of the card.
- Long lines scroll horizontally; preserve indentation with the default `white-space: pre`.
- Body is set in `--mono` (var(--mono)), so it does NOT depend on the bundled Chinese font — code stays readable even before font subsetting.
- The single-ink-blue rule still applies: language tag is stone-colored, the card itself is ivory on parchment, no chromatic accents.

**Overview diagram authoring tips** (`overview_diagram` is the only
diagram that always renders above the conclusions list, so it sets the
reader’s first impression):

- Prefer `graph LR` for pipeline-style architectures, `graph TD` for
  layered / control-flow. `graph LR` with many cross-edges can look
  crowded — if there are 3+ back-edges or >10 nodes, consider splitting
  the overview into two diagrams (one per chapter) and leaving the
  overview as a simpler 5–7 node hub-and-spoke.
- Use `subgraph ... end` to visually group peer components (e.g. "Workers",
  "Storage"). Subgraphs get a faint warm fill and render predictably.
- Avoid raw `&` / `<` / `>` in node labels — escape them with HTML entities
  (`&amp;`, `&lt;`, `&gt;`) or use plain words. Bare `&` will not break
  the render but produces a parser warning.
- A short overview caption (`<div class="diagram-caption">`) is auto-added
  if you set `overview_diagram_caption` in the JSON. Keep it to one line.
...
| Fenced code block | `.code-block` (ivory card, 8px radius, optional language tag, mono) |
